...
class Board():
    def __init__(self, num_rows, num_cols):
        ...
        self.neighbor_table = neighbor_tables[dim] # [fast新增]
        self.corner_table = corner_tables[dim] # [fast新增]
        self.move_ages = MoveAge(self) #初始为-1，表示还没有落子
    ...
    def place_stone(self, player, point):
        ...
        self.move_ages.increment_all() # 每一步棋子的历史
        self.move_ages.add(point) # 新下的棋子，历史为0
...
# tag::apply_zobrist[]
        ...
        self._hash ^= zobrist.HASH_CODE[point, None] # 空白点为什么也要设定hash
...
# end::apply_zobrist[]
    ...
    def _remove_string(self, string):
        for point in string.stones:
            ...
            self._hash ^= zobrist.HASH_CODE[point, None] # [fast新增]

    def is_self_capture(self, player, point): # [speed up][不是是提速重点] 算是go规则细化
        friendly_strings = []
        for neighbor in self.neighbor_table[point]:
            neighbor_string = self._grid.get(neighbor)
            if neighbor_string is None:
                ...
                if neighbor_string.num_liberties == 1:
                    ...
                    return False
        # self catch 指必须把所有自己的string气给堵上。（连通性也合理，更严格的限制，但是对提速没有太大帮助）      
        if all(neighbor.num_liberties == 1 for neighbor in friendly_strings):
            return True
        return False

    # 能吃对面子，就不会ko
    def will_capture(self, player, point): # [speed up][是提速重点] ko check处调用, 提速2-3倍。关掉逻辑后速度退化回goboard版本
        for neighbor in self.neighbor_table[point]:
            neighbor_string = self._grid.get(neighbor)
            if neighbor_string is None:
                continue
...
class GameState():
    ...
    def is_move_self_capture(self, player, move):
        if not move.is_play:
            return False
        return self.board.is_self_capture(player, move.point) # [fast新增检查] 判断下移到board
    ...
    def does_move_violate_ko(self, player, move):
        ...
        # 关掉这段逻辑看看速度掉多少 （和goboard版本一样的速度） !!! [speed up] 3倍的提速在这里
        if not self.board.will_capture(player, move.point): # [fast新增检查]
            return False # 不能吃掉对面的子，说明局面新增了（感觉是上一版本对ko的定义太宽泛了，检查过多导致效率低）
