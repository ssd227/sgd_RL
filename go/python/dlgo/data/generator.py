...
'''
这部分功能可以被 dataset， dataloader取代。
'''

# consolidate_games的优化替代版，功能一致
class DataGenerator:
    def __init__(self, data_directory, samples):
        self.data_directory = data_directory
        self.samples = samples

        #已经被整理好的压缩文件名（部分对局被生成） 例如：# KGS-2009-19-18837-train_labels_0.npy
...
# <2> Depending on the application, we may need to know how many examples we have.
    ...
    def _generate(self, batch_size, num_classes):
        for zip_file_name in self.files: # 很多文件并不是同时加载
            ...
            for feature_file in glob.glob(base):
                ...
                while x.shape[0] >= batch_size:
                    ...
                    yield x_batch, y_batch # yield会hold, 想像成协程
