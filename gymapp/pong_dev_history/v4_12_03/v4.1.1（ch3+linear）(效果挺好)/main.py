...
def train_policy_gradient(env, agent, episodes):
    ...
    for episode in range(episodes):
        ...
        ch3_state = torch.stack([state, torch.zeros_like(state), diff_state], dim=2) # [MxN,C]
        ch3_state_flate = torch.flatten(ch3_state, start_dim=1, end_dim=-1) # [MxN,C]
        
        # 收集1局样本数据
        while True:
            action = agent.select_action(ch3_state_flate)
            # 环境交互         
            next_state, reward, terminated, truncated, info = env.step(action)

            # 收集数据
            agent.collector.record_decision(state=ch3_state_flate, action=action, reward=reward)  # 大部分帧的reward都是0
            ...
            ch3_state = torch.stack([state, last_state, diff_state], dim=2) # MxNxC
            ch3_state_flate = torch.flatten(ch3_state, start_dim=1, end_dim=-1) # [MxN,C]
