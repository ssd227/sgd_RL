...
看来慢不是sgd的问题， 也不是M=10的问题，纯属模型效果好了之后，和环境的交互时间更长，导致整体过程慢
但是本轮的效果确实不错，感觉比24号的训练收敛的快了一点。（不排除是模型初始化参数的问题）

训练完感觉效果又和24号差不多，没有特别显著的提升。
训练700轮，模型又炸了

Episode 725, Total Reward: -17.0
Episode 726, Total Reward: -13.0
Episode 727, Total Reward: -10.0
Episode 728, Total Reward: -16.0
    ^^^^^^
  File "c:\Users\SGD\Desktop\sgd-RL\pong\pong_v2.py", line 191, in main

  File "c:\Users\SGD\Desktop\sgd-RL\pong\pong_v2.py", line 111, in train_policy_gradient
...
训练记录
   使用M=10轮一迭代，效果反而变差了
      1、使用sgd就能改善吗？ 
         ...
         !!! bug修完了，感觉有点起色 !!!
         200轮，感觉就起飞了，难不成是NN初始化的问题
         真的是好麻烦，和炼丹没啥区别了（PG+SGD不是个好组合）

      2、advantage，应该先标准化，还是10轮后标准化
         感觉影响不是太大
      
      3、还是说数据量增多了后，adam的学习率3e-4就不合适了
            加了10倍的数据，学习率是不是相应的减少点
   ...
   for episode in range(episodes):
      ...
      if (episode + 1) % M == 0: 
         ...
         for states_batch, actions_batch, advantages_batch in dl:
            action_probs = policy_net(states_batch.to(device)) # softmax后的概率
            action_log_probs = torch.log(action_probs.gather(1, actions_batch.to(device).unsqueeze(1))) # todo 这行是干嘛的？

            loss = -(action_log_probs.squeeze() * advantages_batch.to(device)).sum() # 定义nn形式的loss
